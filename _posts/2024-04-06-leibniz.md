...
## Rough plan of next
How to differentiate a function. The *derivative* of a function. 

Leibniz wasn't thinking in terms of functions. We don't need to. We will see that *differentials* are quite different. Consider 
* Circle equation
* Apply d
* Equations work. Y not a function of X everywhere or comfortably
* y cubed = (elliptic curve), mostly double valued, but a complete mess.

* Rules
* Dimensions are essential
* What you can integrate, what the derivative is
* Meaning
* Parametrised curve. Map to reals.
* How d works on functions (meaning of df).
* Can pull back any real arithmetic to our differentials. Need dimensions to track because.....
* Dimensionless has no dependency on parameter, also true for is zero equations.

* Mathematical terminology. Germs. Co-germ differentials.
* Higher order
* We have the rules. Second derivative not independent of parameter. Need to make a decision. This relates to curvature and is deep. See Leibniz section. If we pick dx as parameter then ddx=0
* second derivative is now given by usual formula.
* We cannot avoid this dependency (why exactly)?

* Epilogue
* Why not forms. Example suggestions (list)
* What forms are. ddx =0 always not helpful. Problem is that forms only have access to first order information. See definition of tangent space on a manifold. Hence they fail here. You need co-germs for higher order.
* 
...
[Extending the Algebraic Manipulability of Differentials](https://arxiv.org/abs/1801.09553)
They say that for the first derivative "few practical problems in treating differentials
algebraically". They point out that you cannot manipulate higher order differentials and assume that d2x/dt2 or whatever is the second derivative (we agree on this, and it is an essential point). They do give the algebra of differentials and show that there is no need for functional dependence. They give formulas for higher derivatives by repeated application of d. Using that approach one ends up with very complicated expressions (?? are they parameter free)? This account eventually alludes to Leibniz's "progression of variables" but misses out any meaning for individual differentials and focuses on the derivatives of functions (a bit obsessively). It also fails to highlight why progression is different in first and then in higher differentials. Nothing about integration etc.

See later: https://arxiv.org/abs/2210.07958
https://arxiv.org/abs/1811.03459
Leibniz Vindicated: Calculating Reliably with Differentials



https://arxiv.org/abs/2210.07958 
---
@inbook{Isabelle_Fite_2023,
   title={Total and Partial Differentials as Algebraically Manipulable Entities},
   ISBN={9781838811112},
   url={http://dx.doi.org/10.5772/intechopen.107285},
   DOI={10.5772/intechopen.107285},
   booktitle={Operator Theory - Recent Advances, New Perspectives and Applications},
   publisher={IntechOpen},
   author={Isabelle Fite, Maria and Bartlett, Jonathan},
   year={2023},
   month=may }
---
...
* * Thomas's Calculus says dy/dx is not a ratio. Can it be interpreted as a ratio?
  * Russell thought not.
  * Much confusion follows
  * First answer talks about infinitessimal quantities being abandoned so what we have now is a limit (though the limit he gives is for f') and alimit of a quotient isn't the quotient of two limits. SO not a ratio. But it is "very suggestive". They behave as if they were. Differentials aren't dy and dx in dy/dx and we can get around this with ifninitessimals.
  * One comment asks about problems - answer is the second derivative.
  * Jesse Madnick's answer may be on to something (#2 answer). Reference to pushforward map.
  * Another answer - best to think of d/dx as an operator.
